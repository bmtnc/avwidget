#' Create Price/EBIT Multiple Chart
#'
#' Creates a line chart showing historical Price/EBIT multiple evolution
#' using TTM EBIT values
#'
...
#' @return A ggplot2 object containing the Price/EBIT chart
#'
...
create_price_ebit_chart <- function(market_cap_history, financial_data,
  ...
  # Calculate TTM EBIT using rolling 4 quarters
  ttm_ebit <- financial_data %>%
    dplyr::arrange(date) %>%
    dplyr::mutate(
      lag1 = dplyr::lag(ebit, 1),
      lag2 = dplyr::lag(ebit, 2),
      lag3 = dplyr::lag(ebit, 3),
      ttm_ebit = (ebit + lag1 + lag2 + lag3)
    ) %>%
    dplyr::select(date, ttm_ebit)

  # Create date sequence and forward fill TTM EBIT
  ebit_filled <- data.frame(
    date = seq.Date(
    ...
    dplyr::left_join(ttm_ebit, by = "date") %>%
    tidyr::fill(ttm_ebit, .direction = "down") %>%
    dplyr::filter(!is.na(ttm_ebit))

  # Calculate Price/EBIT ratio using market cap in billions
  price_ebit_df <- market_cap_history %>%
    dplyr::left_join(ebit_filled, by = "date") %>%
    dplyr::mutate(
      price_ebit_ratio = market_cap / ttm_ebit,
      # Handle potential negative EBIT
      price_ebit_ratio = ifelse(ttm_ebit <= 0, NA, price_ebit_ratio)
    ) %>%
    dplyr::filter(
      date >= lubridate::as_date(start_date),
      !is.na(price_ebit_ratio)
    )

  # Calculate median for reference line
  median_ratio <- median(price_ebit_df$price_ebit_ratio, na.rm = TRUE)
  # Calculate current ratio for annotation
  latest_ratio <- price_ebit_df$price_ebit_ratio[which.max(price_ebit_df$date)]
  latest_date <- max(price_ebit_df$date)
  latest_date_formatted <- format(latest_date, "%b %d, %Y")

  # Create callout text
  callout_text <- sprintf("%.1fx", latest_ratio)

  # Calculate y position for callout (slightly above the point)
  y_range <- diff(range(price_ebit_df$price_ebit_ratio, na.rm = TRUE))
  callout_y_position <- latest_ratio + (y_range * 0.1)

  # Create the visualization
  ggplot2::ggplot(price_ebit_df, ggplot2::aes(x = date, y = price_ebit_ratio)) +
    ...
    ggplot2::geom_point(
      data = price_ebit_df[price_ebit_df$date == latest_date, ],
      color = "black",
      size = 2
    ) +
    # Add callout text
